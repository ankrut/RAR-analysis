...
X = logspace(-6,2,60);
T = script.beta0.createGrid(vmSeed,X).map(@(vm) model.tov.rar.profile('model',vm));
...
[~,vm] = script.gosect.beta0(...
	'model',	vmSeed,...
	'list',		module.ProfileResponse(@(SOL) searchCfg.ResponseList.rh(SOL)/searchCfg.ResponseList.rc(SOL),'min'),...
	'interval',	[7,10]*1E-1,...
